...
class WxPayApi
{
	public static $notify_url = "/plus/carbuyaction.php?dopost=return";
	/**
	 * 
	 ...
	 * @return 成功时返回，其他抛异常
	 */
	public static function unifiedOrder($inputObj, $payment, $timeOut = 6)
	{
		...
		if(!$inputObj->IsNotify_urlSet()){
			$inputObj->SetNotify_url(self::$notify_url."&code=".$payment['code']);//异步通知url
		}
		
		$inputObj->SetAppid($payment['weipay_appid']);//公众账号ID
		$inputObj->SetMch_id($payment['weipay_mchid']);//商户号
	...
	{		
		...
		if($useCert == true){
			...
			curl_setopt($ch,CURLOPT_SSLCERT, DEDEINC."/cert/apiclient_cert.pem");
			curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
			curl_setopt($ch,CURLOPT_SSLKEY, DEDEINC."/cert/apiclient_key.pem");
		}
		//post提交方式
